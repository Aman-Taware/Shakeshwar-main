...
const PropertyAmenities = ({ amenities = [], propertyId = '' }) => {
  ...
  const [totalImages, setTotalImages] = useState(8); // Default to 8 maximum images to try
      ...
      <div className="mt-12">
        ...
        <div className="relative">
          ...
          <div className="aspect-w-16 aspect-h-9 bg-gray-100 rounded-lg overflow-hidden">
            <img 
              src={`/images/property/${propertyId}/amenities/${propertyId}-amenitie-${currentImageIndex + 1}.jpg`}
              alt={`Property Amenity`}
              className="object-cover w-full h-full"
              onError={(e) => {
                // Try different file extension if jpg fails
                if (e.target.src.endsWith('.jpg')) {
                  e.target.src = `/images/property/${propertyId}/amenities/${propertyId}-amenitie-${currentImageIndex + 1}.jpeg`;
                } else if (e.target.src.endsWith('.jpeg')) {
                  // Try with "pearl" prefix
                  e.target.src = `/images/property/${propertyId}/amenities/pearl-amenitie-${currentImageIndex + 1}.jpg`;
                } else if (e.target.src.includes('pearl-amenitie') && e.target.src.endsWith('.jpg')) {
                  // Try pearl with jpeg extension
                  e.target.src = `/images/property/${propertyId}/amenities/pearl-amenitie-${currentImageIndex + 1}.jpeg`;
                } else if (e.target.src.includes('pearl-amenitie') && e.target.src.endsWith('.jpeg')) {
                  // Try undefined prefix
                  e.target.src = `/images/property/${propertyId}/amenities/undefined-amenitie-${currentImageIndex + 1}.jpg`;
                } else if (e.target.src.includes('undefined-amenitie') && e.target.src.endsWith('.jpg')) {
                  // Try undefined with jpeg extension
                  e.target.src = `/images/property/${propertyId}/amenities/undefined-amenitie-${currentImageIndex + 1}.jpeg`;
                } else {
                  // If all fail, use placeholder
                  e.target.src = '/images/website/placeholder-amenity.jpg';
                  // If we've reached the placeholder and we're at the max image index, adjust totalImages
                  if (currentImageIndex >= totalImages - 1) {
                    setTotalImages(currentImageIndex + 1);
                  }
                }
              }}
            />
